  ...
  df %>%
    mutate(
      {{ new_col_name }} := if_else(
        rowSums(!is.na(across(all_of(columns)))) > 0,
        TRUE,
        NA
      )
    ) %>%
    relocate({{ new_col_name }}, .before = all_of(columns[1]))
}
...
check_box_convert <- function(df, check_box_cols, title = NULL) {
  out <- df %>%
    mutate(across(all_of(check_box_cols), ~ as.logical(.x))) %>%
    rowwise() %>%
    mutate(across(all_of(check_box_cols), ~ {
      if (all(c_across(all_of(check_box_cols)) == FALSE)) NA else .x
    })) %>%
    ungroup()
